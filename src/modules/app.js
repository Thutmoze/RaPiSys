...
// ---------------------------------------------------------------------------
// The server protects mutating endpoints with X-Admin-Token (generated by
// deploy.sh). The upstream UI predates that, so we inject the header into
// EVERY same-origin /api request — fixing the legacy settings panel too —
// and prompt once when a request comes back 401.

function getToken() { return localStorage.getItem('adminToken') || ''; }

function promptForToken() {
  const t = window.prompt(
    'Admin token required.\n\nFind it on your Pi in ~/rapisys/.env (ADMIN_TOKEN=…) or in the deploy.sh install output.');
  if (t && t.trim()) { localStorage.setItem('adminToken', t.trim()); return true; }
  return false;
}

// Inject the token into legacy fetches as well.
const _fetch = window.fetch.bind(window);
window.fetch = (input, init = {}) => {
  try {
    const url = typeof input === 'string' ? input : input.url;
    if (url && (url.startsWith('/api') || url.includes(`${location.host}/api`))) {
      const token = getToken();
      if (token) {
        init.headers = { ...(init.headers || {}), 'X-Admin-Token': token };
      }
    }
...
async function api(path, opts = {}, retried = false) {
  ...
  if (res.status === 401 && !retried) {
    // Wrong or missing token: ask once and retry the same call.
    localStorage.removeItem('adminToken');
    if (promptForToken()) return api(path, opts, true);
  }
