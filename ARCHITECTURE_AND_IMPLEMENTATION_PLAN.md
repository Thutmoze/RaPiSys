...
- **LAN discovery.** Because the container runs `network_mode: host`, it can probe the local /24 directly. `POST /api/nodes/scan` sweeps every address on the local /24 (wider prefixes are skipped rather than queueing 65k probes) and classifies each responder. Identification uses `/api/v1/node-summary` rather than `/api/health`: the health payload is generic enough that any service could return it, whereas the peer endpoint exists only in RaPiSys, and an unauthenticated 401 is as identifying as a 200. Responders are reported as `ready`, `needs-tls` (HTTP only), or `outdated` (RaPiSys without the peer endpoint). Results prefer the reverse-resolved name over the raw IP — raw IPs break on DHCP lease changes, and Pi OS ships Avahi by default.
